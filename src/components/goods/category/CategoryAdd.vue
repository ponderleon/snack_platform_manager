      ...
      <div class="dialog-content-wrapper">
        ...
        <el-form :model="dataForm" :rules="dataRule" ref="dataFormRef" @keyup.enter.native="dataFormSubmit()"
          ...
          <el-form-item label="是否显示" prop="isShow">
            <el-select v-model="dataForm.isShow" placeholder="请选择是否显示" class="selected-input" :disabled="dataForm.categoryLevel < 3">
...
export default {
  data() {
    return {
      ...
      dataForm: {
        name: '',           // 分类名称
        parentId: 0,        // 父分类ID
        categoryLevel: 1,   // 分类层级，默认为1（一级分类）
        isShow: 1,          // 是否显示，默认显示
        sort: 0,            // 排序，默认为0
        icon: '',           // 图标
        productUnit: ''     // 统计单位
  ...
  methods: {
    ...
    /**
     * 初始化添加对话框
     * @param {Number} parentId - 父分类ID，如果没有则为0（添加一级分类）
     * @param {Number} parentLevel - 父分类层级，用于确定新分类的层级
     */
    init(parentId = 0, parentLevel = 0) {
      this.visible = true;
      ...
      this.$nextTick(() => {
        ...
        // 如果没有父分类，则为一级分类
        // 否则为父分类层级+1
        this.dataForm.categoryLevel = parentId ? parentLevel + 1 : 1;

        // 如果有父分类ID，获取父分类信息
        if (parentId) {
          this.loadParentCategoryInfo(parentId);
        } else {
        ...
        // 只有三级分类才允许设置是否显示
        // 一级和二级分类默认显示且不可修改
        if (this.dataForm.categoryLevel < 3) {
    ...
    loadParentCategoryInfo(parentId) {
      this.$http.get(`/data/snack_platform/category/info/${parentId}`).then((data) => {
        ...
        if (data && data.code === 200) {
          ...
          this.$message.error(data?.msg || '获取父分类信息失败');
        }
      }).catch(error => {
        console.error('获取父分类信息失败:', error);
        this.$message.error('获取父分类信息失败');
      });
    },
    ...
    dataFormSubmit() {
      ...
      formRef.validate((valid) => {
        if (valid) {
          // 发送添加请求
          this.$http.post('/data/snack_platform/category/save',this.dataForm).then((data) => {
            if (data && data.code === 200) {
              // 操作成功提示
              this.$message({
                message: '添加成功',
