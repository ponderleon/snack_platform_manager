    ...
    <div class="dialog-center-container">
      ...
      <div class="dialog-content-wrapper">
        ...
        <el-form :model="dataForm" :rules="dataRule" ref="dataFormRef" @keyup.enter.native="dataFormSubmit()"
          ...
          <!-- 商品标题 -->
          <el-form-item label="商品标题" prop="skuTitle">
            <el-input v-model="dataForm.skuTitle" placeholder="请输入商品标题"></el-input>
          </el-form-item>

          <!-- 商品副标题 -->
          <el-form-item label="商品副标题" prop="skuSubtitle">
            <el-input v-model="dataForm.skuSubtitle" placeholder="请输入商品副标题"></el-input>
          </el-form-item>

          <!-- 商品说明 -->
          <el-form-item label="商品说明" prop="skuDescription">
            <el-input v-model="dataForm.skuDescription" placeholder="请输入商品说明" type="textarea" :rows="3"></el-input>
          </el-form-item>

          <!-- 商品默认图片 -->
          <el-form-item label="商品图片" prop="skuDefaultImg">
            <el-input v-model="dataForm.skuDefaultImg" placeholder="请输入商品图片URL"></el-input>
            <div v-if="dataForm.skuDefaultImg" style="margin-top: 10px;">
              <img :src="dataForm.skuDefaultImg" style="max-height: 100px; max-width: 100px;">
            </div>
          </el-form-item>
          ...
          <el-form-item label="销量" prop="saleCount">
            <el-input-number v-model="dataForm.saleCount" :min="0" :step="1"></el-input-number>
          </el-form-item>

          <!-- 库存 -->
          <el-form-item label="库存" prop="stock">
            <el-input-number v-model="dataForm.stock" :min="0" :step="1"></el-input-number>
          </el-form-item>
...
export default {
    ...
    return {
      ...
      dataRule: {
        ...
        saleCount: [
          ...
          { type: 'number', message: '销量必须为数字', trigger: 'blur' }
        ],
        stock: [
        ...
        brandId: [
          ...
          { type: 'number', message: '品牌ID必须为数字', trigger: 'change' }
        ]
      }
    }
  },
  ...
  methods: {
     ...
     * @param {Array} categoryPath - 分类的完整路径数组
     */
     ...
     */
    loadSkuInfo(id) {
      this.$http.get(`/data/snack_platform/skuinfo/info/${id}`).then((data) => {
    ...
    dataFormSubmit() {
      ...
      formRef.validate((valid) => {
        if (valid) {
          ...
          const submitData = {
            ...this.dataForm,
            cascadedCategoryId: this.cascadedCategoryId // 提交分类路径
          };

          // 发送更新请求
          this.$http.put('/data/snack_platform/skuinfo/update', submitData).then((data) => {
            if (data && data.code === 200) {
              // 操作成功提示
              this.$message({
                message: '修改成功',
