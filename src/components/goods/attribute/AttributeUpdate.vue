...
export default {
  ...
  methods: {
    ...
    /**
     * 初始化修改对话框
     * @param {Number} id - 要修改的属性ID
     */
    init(id) {
      if (!id) {
        this.$message.error('未指定要修改的属性ID');
        return;
      }
    ...
    loadAttributeInfo(id) {
      this.$http.get(`/data/snack_platform/attribute/info/${id}`).then((data) => {
        if (data && data.code === 200) {
          ...
          this.$message.error(data?.msg || '获取属性信息失败');
        }
      }).catch(error => {
        console.error('获取属性信息失败:', error);
        this.$message.error('获取属性信息失败');
      });
    },
    ...
    dataFormSubmit() {
      ...
      formRef.validate((valid) => {
        if (valid) {
          ...
          // 发送更新请求
          this.$http.put('/data/snack_platform/attribute/update', this.dataForm).then((data) => {
            if (data && data.code === 200) {
              // 操作成功提示
              this.$message({
                message: '修改成功',
