      ...
      <div class="dialog-content-wrapper">
        ...
        <el-form :model="dataForm" :rules="dataRule" ref="dataFormRef" @keyup.enter.native="dataFormSubmit()"
          ...
          <el-form-item label="分组名称" prop="name">
            <el-input v-model="dataForm.name" placeholder="请输入分组名称"></el-input>
          </el-form-item>

          <!-- 分组说明 - 可编辑 -->
          <el-form-item label="分组说明" prop="description">
            <el-input v-model="dataForm.description" placeholder="请输入分组说明"></el-input>
          </el-form-item>

          <!-- 排序 - 可编辑 -->
          <el-form-item label="排序" prop="sort">
            <el-input v-model.number="dataForm.sort" placeholder="请输入排序值" class="short-input"></el-input>
          </el-form-item>
...
export default {
  data() {
    return {
      ...
      dataRule: {
        ...
        description: [
          { required: false, message: '分组说明', trigger: 'blur' }
        ],
        sort: [
  ...
  methods: {
    ...
    /**
     * 初始化修改对话框
     * @param {Number} id - 要修改的分组ID
     */
    init(id) {
      if (!id) {
        this.$message.error('未指定要修改的分组ID');
        return;
      }
     ...
     */
    loadGroupInfo(id) {
      this.$http.get(`/data/snack_platform/attributegroup/info/${id}`).then((data) => {
        if (data && data.code === 200) {
          // 填充表单数据
          this.dataForm = data.data;

          // 如果后端返回了分类路径，直接使用
          if (data.data.cascadedCategoryId && Array.isArray(data.data.cascadedCategoryId)) {
            // 设置级联路径
            this.cascadedCategoryId = data.data.cascadedCategoryId;
          }

        } else {
          this.$message.error(data?.msg || '获取分组信息失败');
        }
      }).catch(error => {
    ...
    async getCascaderCategoryTree() {
      try {
        ...
        if (data && data.code === 200) {
          ...
          this.$message.error(data?.msg || '分类查询失败');
          return false;
        }
      } catch (error) {
    ...
    dataFormSubmit() {
      ...
      formRef.validate((valid) => {
        if (valid) {
          ...
          // 发送更新请求
          this.$http.put('/data/snack_platform/attributegroup/update', this.dataForm).then((data) => {
            if (data && data.code === 200) {
              // 操作成功提示
              this.$message({
                message: '修改成功',
...
/* 特定短输入框样式 - 排序输入框专用小尺寸 */
.short-input {
  max-width: 6.25rem !important; /* 减小排序输入框宽度 */
}
